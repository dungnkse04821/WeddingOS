...
class EventRemovalPreviewScreen extends StatefulWidget {
  ...
  @override
  State<EventRemovalPreviewScreen> createState() => _EventRemovalPreviewScreenState();
}
...
class _EventRemovalPreviewScreenState extends State<EventRemovalPreviewScreen> {
  ...
  Widget build(BuildContext context) {
    final blockingInvariants = _previewData != null
        ...
        : <String>[];
    final bool isBlocked = blockingInvariants.contains('FINAL_MAIN_EVENT_INVARIANT');

    return Scaffold(
      backgroundColor: const Color(0xFF0F172A),
      appBar: AppBar(
        title: const Text('Xem trước hủy sự kiện', style: TextStyle(fontWeight: FontWeight.bold)),
        backgroundColor: const Color(0xFF1E293B),
        elevation: 0,
      ),
      body: _loading
        ...
        children: [
          ...
          Row(
            children: [
              ...
              Text(
                widget.event['is_main_event'] == true ? 'Sự kiện chính (Main Event)' : 'Sự kiện phụ',
  ...
  Widget _buildDownstreamSummaryCard() {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Ảnh hưởng tài chính & lời mời', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 14)),
        ...
        children: [
          ...
          if (candidates.isEmpty)
            const Padding(
              ...
              child: Text('Không có công việc mẫu nào bị xóa.', style: TextStyle(color: Colors.white38, fontSize: 13)),
            )
          else
            ...candidates.map((task) {
              return Padding(
        ...
        children: [
          const Row(
            children: [
              ...
              SizedBox(width: 8),
              Text('Công việc tự chọn được giữ lại', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 14)),
            ],
          ),
          const SizedBox(height: 6),
          const Text(
          ...
          if (list.isEmpty)
            const Padding(
              padding: EdgeInsets.symmetric(vertical: 8.0),
              child: Text('Không có công việc nào được dời đi.', style: TextStyle(color: Colors.white38, fontSize: 13)),
            )
          else
            ...list.map((task) {
          ...
          children: [
            ...
            Expanded(
              child: ElevatedButton(
                ...
                onPressed: (_previewData == null || _submitting || _loading) ? null : _commitRemoval,
                child: _submitting
                    ? const SizedBox(
                        height: 20,
                        width: 20,
                    ...
                    : const Text('Hủy sự kiện', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
              ),
            ),
          ],
        ),
