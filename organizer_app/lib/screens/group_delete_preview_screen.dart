...
class GroupDeletePreviewScreen extends StatefulWidget {
  ...
  @override
  State<GroupDeletePreviewScreen> createState() => _GroupDeletePreviewScreenState();
}
...
class _GroupDeletePreviewScreenState extends State<GroupDeletePreviewScreen> {
  ...
  Future<void> _loadPreview() async {
    ...
    try {
      final data = await SupabaseService.instance.previewPrimaryGroupDelete(widget.groupId);
      setState(() {
  ...
  Future<void> _handleCommit() async {
    ...
    try {
      await SupabaseService.instance.commitPrimaryGroupDelete(widget.groupId, _impactFingerprint);
      if (mounted) {
      ...
      if (errStr.contains('STALE_IMPACT') || errStr.contains('40001')) {
        ...
        setState(() {
          _errorMessage = 'Lỗi thực thi xóa nhóm: $e';
          _isLoading = false;
        });
      }
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: const Color(0xFF1E293B),
        title: const Text('Xác nhận xóa nhóm', style: TextStyle(color: Colors.white)),
        elevation: 0,
      ),
      body: _isLoading
              ...
              : SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const BulletPoint(text: 'Khách mời thuộc nhóm này sẽ KHÔNG bị xóa khỏi đám cưới.'),
                      const BulletPoint(text: 'Mối quan hệ nhóm của họ sẽ chuyển thành "Chưa gán nhóm" (PrimaryGroup = NONE).'),
                      const BulletPoint(text: 'Tác vụ này không thay đổi thành viên nhóm mời hay số người mời (Invited Count).'),
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            if (_affectedGuests.isNotEmpty) ...[
                              const SizedBox(height: 12),
                              const Text('Danh sách khách mời bị ảnh hưởng:', style: TextStyle(color: Colors.white54, fontSize: 12)),
                              const SizedBox(height: 8),
                              Container(
                                ...
                                child: ListView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                    ...
                                    return Padding(
                                      ...
                                      child: Text('- ${guest['name']}', style: const TextStyle(color: Colors.white70, fontSize: 13)),
                                    );
                                  },
                                ),
                              ),
