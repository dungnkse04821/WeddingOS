...
import 'package:organizer_app/screens/directory_screen.dart';
...
void main() {
  final testGroups = [
    PrimaryGroupModel(id: 'g1', weddingId: 'w1', name: 'Đồng nghiệp', createdAt: DateTime.now()),
    PrimaryGroupModel(id: 'g2', weddingId: 'w1', name: 'Bạn cấp 3', createdAt: DateTime.now()),
  ];

  final testParties = [
    InvitationPartyModel(id: 'p1', weddingId: 'w1', displayName: 'Gia đình bác Tư', invitedCount: 4, createdAt: DateTime.now(), updatedAt: DateTime.now()),
    InvitationPartyModel(id: 'p2', weddingId: 'w1', displayName: 'Anh Nam & bạn', invitedCount: 2, createdAt: DateTime.now(), updatedAt: DateTime.now()),
  ];
  ...
  testWidgets('DirectoryScreen renders Guest List and Party List tabs', (WidgetTester tester) async {
  ...
  testWidgets('PartyCreateEditScreen preserves Invited Count and renders members', (WidgetTester tester) async {
    ...
    expect(
      find.text('Thay đổi danh sách thành viên bên dưới sẽ không tự động làm thay đổi hạn mức Số người mời thực tế đã chốt.'),
      findsOneWidget,
    );

    // Verify members of this party are listed
    expect(find.text('Nguyễn Văn A'), findsOneWidget);
    expect(find.text('Nguyễn Văn B'), findsOneWidget); // unassigned guest is in bottom list
  });
}
